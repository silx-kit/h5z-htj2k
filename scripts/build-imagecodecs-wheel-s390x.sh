#!/usr/bin/env bash
# Build an s390x wheel of imagecodecs (imcd, shared, htj2k, tiff extensions
# only).
#
...
#
# The resulting wheel is written to OUTPUT_DIR (default: ./wheelhouse).

set -euo pipefail

OUTPUT_DIR="${OUTPUT_DIR:-$PWD/wheelhouse}"
OPENJPH_SRC="${OPENJPH_SRC:-$PWD/vendored/OpenJPH}"
OPENJPH_BUILD_DIR="$(mktemp -d)"
trap 'rm -rf "$OPENJPH_BUILD_DIR"' EXIT

apt-get update -q -y
apt_packages=(
  build-essential pkg-config ca-certificates cmake ninja-build
...
IC_SHIM_DIR="$(mktemp -d)"
trap 'rm -rf "$OPENJPH_BUILD_DIR" "$IC_SHIM_DIR"' EXIT
...
pip3_wheel_args=(
  ...
  --wheel-dir "$OUTPUT_DIR" imagecodecs
)
